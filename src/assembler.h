...
public:
  ...
  void EmitCode(uint32_t c) { return EmitData((uint8_t*)&c, sizeof(uint32_t)); }
  void EmitCodeListing(std::initializer_list<uint8_t> l) {
    return EmitData((uint8_t*) std::data(l), l.size());
  }

  void PatchCode(uintptr_t p, uint32_t c) { return PatchData(p, (uint8_t*) &c, sizeof(uint32_t)); }
  ...
  inline uintptr_t BaseAddr() const {
    return (uintptr_t) m.mem;
  }
